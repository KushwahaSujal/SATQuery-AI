  ...
  return (
    <div className="bg-[#050911] text-slate-200 antialiased font-sans h-screen overflow-hidden flex flex-col selection:bg-cyan-500 selection:text-black">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Main Content Area */}
        <main className="flex-1 flex flex-col min-w-0 bg-[#060a14] overflow-hidden">
          {/* Title & Filters Bar */}
          <div className="p-6 pb-3 border-b border-[#142032] shrink-0">
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="bg-[#0b1220] border border-[#1a2942] rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-300 placeholder-slate-500 focus:outline-none focus:border-cyan-500 w-52"
                    placeholder="Search history..."
                    type="text"
                  />
                </div>
                <button className="p-1.5 rounded-lg border border-[#1a2942] bg-[#0b1220] text-slate-400 hover:text-white transition">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg font-medium transition shrink-0 cursor-pointer ${
                      active
                        ? "bg-[#0f283d] text-cyan-400 border border-cyan-500/40 shadow-sm"
                        : "bg-[#0b1220] text-slate-400 border border-[#17253b] hover:text-slate-200"
                    }`}
                  >
                    <span>{tab}</span>
                    <span
                      className={`ml-0.5 px-1.5 py-0.2 rounded-full text-[10px] ${
                        active ? "bg-cyan-500/20 text-cyan-300 font-semibold" : "bg-slate-800 text-slate-300"
                      }`}
                    >
                      {tabCounts[tab] ?? 0}
                    </span>
              ...
              return (
                <article
                  ...
                  className={`relative flex items-center justify-between p-3.5 rounded-xl transition-all cursor-pointer ${
                    isSelected
                      ? "bg-[#091424] border-2 border-cyan-400 shadow-[0_0_20px_rgba(0,229,255,0.12)]"
                      : "bg-[#09101d] border border-[#16233b] hover:border-slate-700"
                  }`}
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    <input
                      ...
                      onChange={() => {}}
                      className="w-4 h-4 rounded bg-[#070e1a] border-cyan-500 text-cyan-500 focus:ring-0 focus:ring-offset-0 cursor-pointer"
                    />

                    {/* Dual or Single Thumbnails */}
                    {item.dualThumb ? (
                      <div className="flex items-center gap-1 shrink-0">
                        <div className={`w-14 h-14 rounded-lg overflow-hidden border border-[#1b2b46] ${item.dualThumb.opt} relative shadow-inner`}>
                          <span className="absolute bottom-0.5 left-1 text-[8px] font-mono text-cyan-300 bg-black/60 px-1 rounded">OPT</span>
                        </div>
                        <div className={`w-14 h-14 rounded-lg overflow-hidden border border-[#1b2b46] ${item.dualThumb.sar} relative shadow-inner`}>
                          <span className="absolute bottom-0.5 left-1 text-[8px] font-mono text-slate-300 bg-black/60 px-1 rounded">SAR</span>
                        </div>
                      </div>
                    ) : (
                      <div className={`w-28 h-14 rounded-lg overflow-hidden border border-[#1b2b46] ${item.singleThumb || "thumb-forest"} shrink-0 relative shadow-inner`} />
                    )}

                    {/* Analysis Details */}
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="px-2 py-0.5 text-[10px] font-medium rounded bg-cyan-950/70 border border-cyan-500/30 text-cyan-300">
                          {item.type}
                        </span>
                      ...
                      <div className="flex items-center gap-3 mt-2 text-[11px] text-slate-400">
                        ...
                        <div className="flex items-center gap-1.5">
                          {item.tags.map((tag) => (
                            <span key={tag} className="px-1.5 py-0.5 rounded bg-[#0b172a] border border-[#1f314f] text-[10px] text-slate-300">
                              {tag}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="flex items-center gap-5 shrink-0 pl-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="relative w-11 h-11 flex items-center justify-center">
                        ...
                        <span className="absolute text-[11px] font-bold text-white">{item.confidence}%</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-400">Confidence</span>
                        {item.status === "Completed" ? (
                          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-emerald-400 bg-emerald-950/40 border border-emerald-500/30 px-2 py-0.5 rounded-full mt-0.5">
                    ...
                    <div className="flex items-center gap-1 text-slate-400">
                      <button className="p-1.5 hover:text-white rounded hover:bg-[#11233d] transition" title="View">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
                          <path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
                        </svg>
                      </button>
                      <button className="p-1.5 hover:text-white rounded hover:bg-[#11233d] transition" title="Download">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
                        </svg>
                      </button>
                      <button className="p-1.5 hover:text-white rounded hover:bg-[#11233d] transition" title="More">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <aside
          className="w-[380px] border-l border-[#142032] bg-[#070c17] flex flex-col justify-between overflow-y-auto shrink-0 select-none"
          data-purpose="history-detail-drawer"
        >
          <div className="p-4 space-y-4">
            {/* Drawer Header */}
            <div className="flex items-center justify-between pb-2 border-b border-[#142032]">
              ...
              <span className="inline-flex items-center gap-1 text-[10px] font-medium text-emerald-400 bg-emerald-950/50 border border-emerald-500/30 px-2.5 py-0.5 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> {selectedItem.status}
              </span>
            </div>

            {/* Large False-Color Map Preview Card */}
            <div className="relative h-44 rounded-xl border border-[#1b2f4d] hero-map-overlay overflow-hidden shadow-lg p-2.5 flex flex-col justify-between">
              ...
              <div className="bg-black/75 backdrop-blur-md p-2 rounded-lg border border-white/10 self-end space-y-1">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-sm bg-red-500" />
                  <span className="text-[10px] text-slate-200 font-medium">Built-up area</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-sm bg-emerald-500" />
                  <span className="text-[10px] text-slate-200 font-medium">Vegetation</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-sm bg-blue-500" />
                  <span className="text-[10px] text-slate-200 font-medium">Water body</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-2.5">
                {selectedItem.tags.map((tag) => (
                  <span key={tag} className="px-2 py-0.5 rounded bg-[#0e192c] border border-[#1c3050] text-[10px] text-slate-300">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            {/* Results Summary */}
            <div className="pt-2 border-t border-[#142032]">
              <h4 className="text-xs font-semibold text-slate-300 mb-2.5">Results Summary</h4>
              <div className="bg-[#0a1220] border border-[#16233a] rounded-xl p-3 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="relative w-10 h-10 flex items-center justify-center">
                      ...
                      <span className="absolute text-[11px] font-bold text-white">{selectedItem.confidence}%</span>
                    </div>
                    <span className="text-xs text-slate-300 font-medium">Confidence</span>
                  </div>
                  <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-400 bg-emerald-950/50 border border-emerald-500/30 px-2.5 py-0.5 rounded-full">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> {selectedItem.status}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 pt-1">
                  <div className="bg-[#0d1728] border border-[#1a2b45] p-2 rounded-lg text-center">
            ...
            <div className="pt-2 border-t border-[#142032]">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-xs font-semibold text-slate-300">Input Images</h4>
                <Link href="/analysis" className="text-[11px] text-cyan-400 hover:underline flex items-center gap-0.5">
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 bg-[#0a1220] border border-[#16233a] rounded-lg p-2">
                  <div className="h-16 rounded overflow-hidden thumb-forest relative border border-slate-700/50" />
                  <div className="flex items-center justify-between mt-1.5">
                    <span className="text-[10px] font-mono text-slate-400">Optical (T1)</span>
                  </div>
                </div>
                <div className="flex-1 bg-[#0a1220] border border-[#16233a] rounded-lg p-2">
                  <div className="h-16 rounded overflow-hidden thumb-sar-1 relative border border-slate-700/50" />
                  <div className="flex items-center justify-between mt-1.5">
                    <span className="text-[10px] font-mono text-slate-400">SAR (T2)</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-[#142032]">
              <div className="flex items-center justify-between py-1">
                <h4 className="text-xs font-semibold text-slate-300">Model Details</h4>
                <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path d="M5 15l7-7 7 7" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
                </svg>
              </div>
              <div className="space-y-1.5 text-xs mt-2 bg-[#09101d] p-2.5 rounded-lg border border-[#142238]">
                <div className="flex justify-between">
                  <span className="text-slate-400">Task</span>
                  <span className="text-slate-200 font-medium">{selectedItem.type}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Model</span>
                  <span className="text-slate-200 font-medium">Remote Sensing VLM (BigEarthNet)</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Parameters</span>
                  <span className="text-slate-200 font-medium">Default (Auto)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 border-t border-[#142032] space-y-2 bg-[#060a13]">
            <Link
              href="/analysis"
              className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-semibold text-xs transition shadow-[0_0_15px_rgba(0,229,255,0.3)]"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-2">
              <Link
                href="/reports"
                className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg border border-[#1d2d47] bg-[#0b1322] hover:bg-[#101b30] text-slate-300 text-xs font-medium transition"
              >
              ...
              <button
                ...
                className="p-2 rounded-lg border border-[#1d2d47] bg-[#0b1322] hover:bg-[#101b30] text-slate-400 hover:text-white transition"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <circle cx="12" cy="5" r="1.5" /><circle cx="12" cy="12" r="1.5" /><circle cx="12" cy="19" r="1.5" />
                </svg>
