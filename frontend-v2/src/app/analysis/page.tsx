...
function SugIcon({ name, className }: { name: string; className?: string }) {
  const m: Record<string, React.ReactNode> = {
    ...
    "arrow-right": <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" strokeLinecap="round" strokeLinejoin="round" /></svg>,
  };
  return m[name] ?? null;
}
...
export default function AnalysisPage() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#08101d] overflow-y-auto px-6 py-4 space-y-4 h-[calc(100vh-64px)]">
      {/* Hero Banner */}
      <section className="relative overflow-hidden rounded-2xl border border-[#182c47] bg-gradient-to-r from-[#0a182a] via-[#0c1f38] to-[#071322] p-5 shrink-0">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="max-w-md space-y-2">
            <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-cyan-500/10 border border-cyan-400/30 text-cyan-400 text-[10px] font-semibold">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">Ask questions, analyze imagery, get insights — powered by advanced AI and real satellite data.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 mt-4 pt-4 border-t border-[#16273e]">
          {[
            ...
            <div key={item.title} className="p-2.5 rounded-xl bg-[#0b1828]/90 border border-[#182e49] hover:border-cyan-500/50 hover:bg-[#0e2137] transition cursor-pointer flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center shrink-0">
                <SugIcon name={item.icon} className="w-4 h-4 text-cyan-400" />
              </div>
      ...
      <section className="flex-1 flex flex-col items-center justify-center py-6 px-4">
        <div className="w-10 h-10 rounded-full bg-cyan-500/10 border border-cyan-400/30 flex items-center justify-center mb-3 glow-cyan-sm">
          <svg className="w-5 h-5 text-cyan-300" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l2.4 6.6L21 11l-6.6 2.4L12 20l-2.4-6.6L3 11l6.6-2.4L12 2z" /></svg>
        </div>
        <h2 className="text-lg font-bold text-white text-center">How can I help you today?</h2>
        <p className="text-xs text-slate-400 text-center max-w-md mt-1 mb-6">Ask anything about your satellite data — from land cover analysis to change detection.</p>
        ...
        <div className="grid grid-cols-2 gap-3 w-full max-w-2xl">
          {SUGGESTIONS.map((s) => (
            <button
              ...
              className="flex items-center gap-3 p-3 text-left rounded-xl bg-[#091524] border border-[#162a42] hover:border-cyan-500/40 hover:bg-[#0c1d32] transition group"
            >
              <SugIcon name={s.icon} className="w-4 h-4 text-cyan-400 shrink-0 group-hover:scale-110 transition-transform" />
              <span className="text-xs text-slate-300 group-hover:text-white font-medium">{s.label}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      <div className="mt-auto shrink-0">
        <div className="rounded-xl border border-[#1a304e] bg-[#091423] p-3 shadow-lg">
          <div className="flex items-center gap-3 pb-2.5">
            ...
            />
            <button onClick={handleSubmit} className="w-8 h-8 rounded-lg bg-gradient-to-r from-cyan-500 to-sky-500 flex items-center justify-center hover:brightness-110 transition shadow-[0_0_10px_rgba(6,182,212,0.5)]">
              <SugIcon name="send" className="w-4 h-4 text-[#041019]" />
            </button>
          </div>
          <div className="flex items-center justify-between pt-2 border-t border-[#132338] text-[11px]">
            <div className="flex items-center gap-2">
              <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#0e1f34] border border-[#1b3452] text-slate-300 hover:text-white hover:border-cyan-500/40 transition">
                <SugIcon name="image" className="w-3.5 h-3.5 text-cyan-400" />
                <span>Attach Image</span>
              </button>
              <button className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#0e1f34] border border-[#1b3452] text-slate-300 hover:text-white hover:border-cyan-500/40 transition">
                <SugIcon name="database" className="w-3.5 h-3.5 text-cyan-400" />
                <span>Select Dataset</span>
              </button>
              <button className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#0e1f34] border border-[#1b3452] text-slate-300 hover:text-white hover:border-cyan-500/40 transition">
                <SugIcon name="target" className="w-3.5 h-3.5 text-cyan-400" />
                <span>Choose Analysis Type</span>
              </button>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-slate-400 font-medium">Advanced</span>
              <div className="w-8 h-4 bg-[#142840] border border-[#223d60] rounded-full p-0.5 flex items-center cursor-pointer">
        ...
        <div className="flex items-center gap-3 text-[10px] text-slate-400 mt-2 px-1">
          <div className="flex items-center gap-1.5">
            <div className="w-3.5 h-3.5 rounded-full bg-emerald-500/20 border border-emerald-400/50 flex items-center justify-center text-emerald-400">
              <SugIcon name="check" className="w-2.5 h-2.5" />
            </div>
