"""
SatQuery AI — ChangeFormerV6 network, vendored from upstream.

Source : https://github.com/wgcban/ChangeFormer (MIT License, (c) 2023 Chaminda Bandara)
Commit : afd1b7ed640aa265a2c730de958416ae7356a2f9
Files  : models/ChangeFormer.py, models/ChangeFormerBaseNetworks.py

Only the 15 definitions in ChangeFormerV6's dependency closure are included, each
copied verbatim (the `# --- file:line ---` markers give its upstream location).
The single change is the timm import path (`timm.models.layers` -> `timm.layers`,
the non-deprecated location in timm>=1.0); behaviour is identical.

Why vendored rather than reimplemented: the previous in-repo reimplementation
matched all 373 parameter names, so strict loading succeeded, but it computed
something else (e.g. num_heads [1,2,5,8] vs upstream [1,2,4,8]). Measured on 200
LEVIR-CD test pairs with the same checkpoint: IoU 0.019 in-repo vs 0.726 upstream.
Matching names is not matching computation — see project/qna.md.

forward(x1, x2) returns a list of 5 logit maps, coarsest first; the last is the
full-resolution (H, W) prediction with 2 classes (0 unchanged, 1 changed).
"""
# ruff: noqa
import math
import warnings
...
# --- models/ChangeFormerBaseNetworks.py:85 ---
...
# --- models/ChangeFormerBaseNetworks.py:98 ---
...
# --- models/ChangeFormerBaseNetworks.py:108 ---
