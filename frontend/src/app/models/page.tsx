          ...
          return (
            <div key={model.name} className="flex items-start gap-4 px-4 py-4 hover:bg-[#0d0d0d] transition-colors">
              ...
              <div className="pt-0.5 shrink-0 w-2 flex justify-center">
                <span className={["status-dot mt-1.5", status === "LOADED" ? "status-dot-green" : status === "AVAILABLE" ? "status-dot-muted" : "status-dot-red"].join(" ")} />
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3">
                  ...
                  <span className={["font-mono-data text-[10px]", lifecycleStyle[status] ?? "text-[#333]"].join(" ")}>
                    {status}
                  </span>
                </div>
                <div className="flex items-center gap-4 mt-0.5">
                  <span className="font-mono-data text-[11px] text-[#404040]">{model.task ?? "unknown"}</span>
                  {model.device && (
